...
def validate_transcription(blocks: object, slide_number: int, depth: int = 1) -> None:
    ...
    for block in blocks:
        if not isinstance(block, dict):
            raise TypeError("Chaque rubrique de transcription doit être un objet.")
...
def render_precision(slide: dict) -> str:
...
def render_v1_index(slides: list[dict]) -> str:
    ...
    <div class="miweb-page-header">
      ...
      <p>{esc(BASELINE)}. Les transcriptions et le discours oral accompagnent chaque slide.</p>
    </div>
...
def render_markdown(slides: list[dict]) -> str:
    parts = [
        ...
        "",
        f"<!-- PDG-LARGE-FILE-JUSTIFICATION: export complet des {len(slides)} transcriptions et discours dans leur ordre, également distribué dans le ZIP. -->",
        "",
    ]
    for slide in slides:
        parts.extend(
...
SOURCE_DESCRIPTIONS = {
    "storyboard.md": "storyboard ayant guidé les 18 visuels finaux.",
    "edition.md": "repères éditoriaux, limites et justification des fichiers publiés.",
    "provenance.json": "correspondances des 18 images et empreintes SHA-256 des JPEG.",
