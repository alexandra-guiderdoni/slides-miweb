...
class DiscoursTest(unittest.TestCase):
    ...
    def test_all_slides_expose_distinct_transcription_and_discours(self):
        ...
        for slide in self.slides:
            ...
            first_line = html.escape(
                slide["notes_orateur"].strip().splitlines()[0], quote=True
            )
    ...
    def test_price_update_keeps_visible_source_text(self):
        ...
        for output in (
            self.build.render_v1_index(self.slides),
    ...
    def test_quote_and_code_markers_are_rendered_without_injecting_html(self):
        ...
        self.assertIn(
            "<blockquote><p>Va Pas Te Croire Supérieur</p></blockquote>", output
        )
    ...
    def test_markdown_links_bold_and_bare_urls_render_safely(self):
        slide = {
            "notes_orateur": (
                "Vérifier l’**authentification** et lire "
                "[la documentation](https://example.org/auth).\n\n"
                "Source complémentaire : https://example.org/guide."
            )
        }
        output = self.build.render_discours(slide)

        self.assertIn("<strong>authentification</strong>", output)
        self.assertIn(
            '<a href="https://example.org/auth">la documentation</a>',
            output,
        )
        self.assertIn(
    ...
    def test_published_assets_and_notes_match_provenance(self):
        ...
        for slide, source in zip(self.slides, provenance["slides"]):
            with self.subTest(slide=slide["numero"]):
                ...
                self.assertEqual(
                    hashlib.sha256(image).hexdigest(), source["published_sha256"]
                )
    ...
    def test_editorial_precisions_remain_visible_with_accordions_closed(self):
        for number in (39, 42, 44):
            ...
            self.assertLess(
                rendered.index('class="fr-callout"'),
                rendered.index('class="fr-accordions-group"'),
            )
        slide = self.slides[41]
        for output in (
            self.build.render_slide(slide, 44),
            self.build.render_markdown(self.slides),
        ):
            self.assertIn("https://www.opquast.com/a-propos/faq/", output)
            self.assertIn(
                "https://www.opquast.com/metiers/ecoles-et-centres-de-formation/",
                output,
            )
