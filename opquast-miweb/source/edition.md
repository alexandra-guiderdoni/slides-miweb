...
`slides.json` est la source de génération des pages et du Markdown téléchargé. Chaque slide comporte une alternative courte, une description du visuel, les textes visibles, un message et le discours oral extrait des notes du PPTX.
...
Le jeu est créé depuis `matrice-slide-ai/create_variant.py`. Son générateur autonome ajoute uniquement le discours oral et la précision tarifaire au modèle existant. L’accueil racine est géré séparément par `matrice-slide-ai/publish_variant.py`.
...
Contrôle ciblé des sorties générées et de la conservation des sources : `PDG self-check, not independent review`. Les générateurs de la matrice, le générateur du jeu et ses tests ont été inspectés. Le modèle existant est étendu uniquement pour les discours et la précision tarifaire ; le code de navigation et les anciennes variantes sont conservés.
...
| Régression | `scripts/validate_variant.sh opquast-miweb` | 13 tests réussis ; HTML validé par html-validate et vnu. |
